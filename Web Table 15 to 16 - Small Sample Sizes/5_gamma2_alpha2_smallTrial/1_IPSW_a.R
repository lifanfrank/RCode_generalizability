...
# source functions
dir<-"D:/A1 Methods Research/Causal Inference/DR Generalizability/R4_Revision/functions/"
source(paste0(dir,"simdata.R"))
source(paste0(dir,"IPSW_est.R"))
...
for(i in 1:nsim){
  set.seed(100+i)
  data<-simdata(gamma,alpha,m=4000)
  N<-data$N
  trial<-data$trial
  cohort<-data$cohort
  both<-data$both
  ...
  Zp<-cbind(1,both$Z1,both$Z2,both$Z3)   # (both) covariate vector for sampling
  Ze<-cbind(1,both$Z1,both$Z2)  # (both) covariate vector for treatment PS
  X<-trial$X                    # (trial) treatment indicator

  res1<-IPSW1(Y,S,Zp,Ze,X,N)
  res2<-IPSW2(Y,S,Zp,Ze,X,N)
